...
const LEFT_N  = 3     // left column count
...
// Left column: 3 tiles centered on the same midpoint
const leftX = MARGIN
const leftRowY = Array.from({length: LEFT_N}, (_, i) => {
  const offset = i - Math.floor(LEFT_N / 2)
  return rightSpineMidY + offset * ROW_GAP
})
const leftTiles = leftRowY.map(cy => ({ x: leftX, y: cy - TILE / 2 }))

// Left spine: to the right of left tiles, with gap
const leftSpineX = leftX + TILE + SPINE_GAP

// oxfs: centered vertically on rightSpineMidY
const oxfsGap = 120 // horizontal gap from left spine to oxfs left edge
...
// C: middle row horizontal (from left spine to oxfs)
...
leftTiles.forEach((t, i) => {
  ...
  push(iconG(t.x, t.y, leftDefs[i].icon))
})
